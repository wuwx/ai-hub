...
- 多租户团队治理
  ...
  - 审计日志（基于 spatie/laravel-activitylog）：记录 API Key 创建/吊销/轮换等关键操作，归属走 causer、上下文存于 properties
- 可靠性与风控
...
- `app/Jobs`: 月度账单生成队列 Job
- `app/Filament`: 管理后台资源与运营看板（含审计日志查看、Profit Dashboard）
- `app/Notifications/Teams`: 配额告警、异常检测、团队邀请、余额不足、账单逾期通知
- `app/Models`: ApiKey、LlmModel/Provider、Plan（含 Model/Provider Entitlements）、QuotaPolicy、RequestLog、UsageLedger、User 等（活动日志直接使用 spatie/laravel-activitylog 内置 `Activity` 模型）
- `database/migrations`: 网关、用量、配额、账单、钱包、活动日志（spatie activity_log）、Passkeys 等表结构
...
- `tests/Feature`: 网关/用量/计费/后台/团队/前台页面/审计日志/导出/认证/Passkey 等功能测试
...
检查预付费团队钱包余额，低于阈值时向 team owner 发送邮件通知（每日去重）。
...
- **Gateway Configuration**：Llm Providers、Llm Models（含 `fallback_model_id` 降级配置）、Plans（含 Model/Provider Entitlements）
...
- 配置 Redis 作为缓存与限流存储
- 开启队列 Worker（`php artisan queue:work`）处理月度出账等异步任务
