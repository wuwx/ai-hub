...
- 用户前台
  ...
  - **Billing 账单与订阅**：查看计划、Stripe Checkout 订阅升级、钱包充值（Top Up）、交易流水、账单 PDF 下载
...
- `app/Http/Middleware`: API Key 鉴权、限流（含 Rate Limit 响应头）、IP 白名单校验
- `app/Http/Controllers/Gateway`: 兼容网关控制器
- `app/Http/Controllers/Billing`: Stripe Webhook、钱包充值
...
- `app/Filament`: 管理后台资源与运营看板（含审计日志查看）
- `app/Notifications/Teams`: 配额告警、异常检测、团队邀请、余额不足、账单逾期通知
- `database/migrations`: 网关、用量、配额、账单、钱包、审计日志、Webhook 端点、Webhook 投递日志相关表结构
- `resources/views/pages/`: Livewire 匿名页面组件（Dashboard、API Keys、Playground、Usage、Billing、Request Logs）
...
- `tests/Feature`: 网关/用量/计费/后台/团队/前台页面/审计日志/Webhook/导出等功能测试（387 个测试用例）
...
所有投递记录（含请求 payload、响应状态码、响应体、延迟、错误信息）存储在 `webhook_deliveries` 表，可在后台查看。连续 10 次失败的端点会被自动禁用。
...
使用 Filament 进行运营管理（Provider / Model / Entitlement / Quota / API Key / Logs / Billing / Wallet / Audit Logs / MCP Servers）。
