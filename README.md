...
- 鉴权与账户
  - API Key 鉴权（Bearer 或 `x-api-key`，基于 Sanctum Personal Access Token）
...
- 订阅与配额
  ...
  - Token 配额（日 / 周 / 月），超出由 `App\Actions\Quota\EnforceTokenQuota` 拦截并抛出 `QuotaExceededException`
  - 订阅变更时通过 `App\Actions\Billing\SyncQuotaFromSubscription` 同步配额
- 模型定价
...
- 可靠性与风控
  - 上游重试与熔断（Circuit Breaker，按 Provider 维度开合）
  - 幂等键缓存与冲突保护
  - 请求内容过滤（`App\Actions\Gateway\ContentFilter`，对 prompt / response 做安全筛查）
...
- `app/Actions/Quota`: 配额校验（token 限额）
...
```bash
php artisan test --compact tests/Feature/Gateway tests/Feature/Quota tests/Feature/ApiKeys tests/Feature/Billing tests/Feature/LlmPlatformAdminSetupTest.php
```
