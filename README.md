...
- 用户前台
  ...
  - **API Keys 管理**：创建、查看、轮换（Rotate）、删除 API Key，支持模型白名单与日限额配置
  - **Usage 用量页**：按日/周/月三粒度查看团队 token 消耗与费用明细
  - **Request Logs**：API 请求日志查询，支持按状态码、模型筛选与分页
  - **Billing 账单与订阅**：查看计划、Stripe Checkout 订阅升级、钱包充值（Top Up）、交易流水
  - **团队邀请**：邮件邀请 + 站内接受邀请流程
...
- `resources/views/pages/`: Livewire 匿名页面组件（Dashboard、API Keys、Usage、Billing、Request Logs）
- `resources/views/welcome.blade.php`: 产品 Landing Page
- `tests/Feature`: 网关/用量/计费/后台/团队/前台页面功能测试（302 个测试用例）
